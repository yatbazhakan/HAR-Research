            ...
            with self.subTest(config=config_file):
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        # Check that CUDA output has the correct shape
                        expected_shape = (input_shape[0], config['num_classes'])
                        self.assertEqual(output_cuda.shape, expected_shape)
                        
                        # Check that CUDA output is not NaN or Inf
                        self.assertFalse(torch.isnan(output_cuda).any())
                        self.assertFalse(torch.isinf(output_cuda).any())
